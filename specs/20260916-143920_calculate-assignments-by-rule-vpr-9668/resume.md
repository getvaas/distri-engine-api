...
- [x] **Phase 4: `toAccountId` real (owner → cuenta)** — investigando VPR-9669 (persistencia) surgió que no existe ningún lookup dinámico owner→cuenta en el sistema real (se configura a mano por deal); se agregó `toAccountId: Long` a `ComponentOwnerRule` y `accountId: Long` a `Assignment`, con fallo explícito si una regla o el remanente no tienen cuenta real resuelta.
